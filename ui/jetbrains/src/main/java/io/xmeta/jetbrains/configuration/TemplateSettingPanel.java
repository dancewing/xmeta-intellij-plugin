...
import io.xmeta.generator.model.Template;
import io.xmeta.generator.model.TemplateGroup;
import io.xmeta.generator.util.TemplateFileUtils;
...
import io.xmeta.jetbrains.configuration.base.BaseGroupPanel;
import io.xmeta.jetbrains.configuration.base.BaseItemSelectPanel;
import io.xmeta.jetbrains.configuration.base.TemplateEditor;
...
public class TemplateSettingPanel implements Configurable {
    ...
    private String findExistedGroupName(String groupName) {
        ...
        if (!group.containsKey(groupName)) {
            if (group.containsKey(ConfigService.DEFAULT_NAME)) {//尝试使用默认分组
                return ConfigService.DEFAULT_NAME;
            } else {
                //获取第一个分组
                return group.keySet().stream().findFirst().orElse(ConfigService.DEFAULT_NAME);
            }
        }
        return groupName;
    }
